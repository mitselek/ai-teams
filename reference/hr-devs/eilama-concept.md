...
The team already uses `~/.claude/teams/hr-devs/inboxes/<name>.json` as the
...
Claude agent (Sven/Tess/etc.)
    │  SendMessage → eilama
    ▼
~/.claude/teams/hr-devs/inboxes/eilama.json
    │  (polled every 2s)
    ▼
eilama-daemon.py
    │  POST /api/generate
...
eilama-daemon.py
    ...
    ▼
~/.claude/teams/hr-devs/inboxes/<requester>.json
```
