...
## [GOTCHA] 2026-03-13 — D1 BLOB käsitlus (dev-toolkit#11)

- SQL statement limit ~100KB — hex literals (`X'...'`) ebaõnnestuvad >50KB failidel
- D1 REST API bind params on alati TEXT — base64 string parameetrina salvestub TEXT-ina (`typeof = 'text'`)
- `{"base64": "..."}` formaat töötab ainult Workers runtime'is, mitte REST API-s (salvestab `[object Object]`)
- **Lahendus:** `INSERT INTO t VALUES (?, unhex(?), ...)` koos hex-encoded string bind params → tegelik BLOB tüüp
- **PRAGMA reminder:** `PRAGMA foreign_keys = OFF` ja `defer_foreign_keys = ON` on D1-s mõlemad no-op CASCADE action'ite vastu
