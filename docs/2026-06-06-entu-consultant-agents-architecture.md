...
Entu ships a small set of **product-native consultant agents** that integrators "hire" into their dev teams. These are not chatbots or doc-search wrappers; they are specialists whose every domain claim is backed by a **referenced, verifiable source in a competency index** — not by training data. The index is the spine: it is queryable, diffable, PR-able, and **auditable before hiring**. Agents are thin consumers of the index. When an agent must reach past its backed knowledge, it labels the answer and **files a PR** against Entu docs, closing a self-improving loop: merged PR → updated index → every agent smarter.
...
- **`partial`** — backed but with a caveat: probe-only (perishable), or a `src` confirm that a `docs`/`openapi` ref *contradicts* (code wins, so the claim still holds — but the doc disagreement is recorded and is itself a gap to PR). Labeled so the integrator knows the edge.
...
### 3.3 Gap-detection loop — biased to PRs, not issues
...
2. **Files a PR against Entu docs** — not merely an issue.

**Why a PR and not an issue (this is a #42-specific design decision, not a default):** #42 documents the observed asymmetry — *Entu PRs land (13/14 merged); issues sit (11 open, weeks, zero response)*. A gap that becomes an *issue* is a request for someone else to write docs, and it waits. A gap that becomes a *PR* is the docs text already written, ready to merge. The gap loop generates the actionable form. Concretely: the agent writes the missing doc section (its own best evidence-backed draft, clearly marked as proposed), opens the PR against `entu/www` (docs) or the relevant repo, and references the consult that surfaced it.

The gap loop is a **three-rung ladder**, choosing the rung by what the gap actually needs:

1. **PR** — when the gap is *doc text the agent can write* (the default; the S3-delete case). Lands fast (PRs merge), pre-written, evidence-backed.
2. **Issue** — when the gap is *a change only a maintainer can make* (a feature request, a wire-format fix in code) — #42 itself flags `entu/api` #39/#40 as issue-only. The honest fallback; not everything is PR-able.
3. **Escalate-to-Argo (apex)** — when the gap is *a question only the maintainer can answer* (a disputed claim with no code value to settle it; the JWT 12h/48h case). The agent escalates to Argo, and **his direct answer (a `file:line` pin or authoritative statement) is recorded back into the index as `maintainer-authoritative` evidence** (`verifier: argo`) — apex-backing the claim for the whole pool, permanently, regardless of other evidence. This is the strongest realization of #42's "Argo gets prioritized signal": his scarce attention is captured *once per gap* as top-tier backing every agent inherits. (Cal's schema §2a/§2b.)

Leaning hard on PR-over-issue is the Entu-specific sharpening of the general contract (Cal's schema permits all three with a preference); rung 3 is new in v2 and is what makes a *disputed* claim resolvable rather than permanently `[GAP]`.
...
`consult surfaces gap → agent files the right rung (PR / issue / escalate-to-Argo) → maintainer merges or answers → index refreshes → every agent that queries that claim is now smarter.` Each real engagement either validates the index or improves it. Over many engagements the index converges toward truth, and the improvement is **driven by real integrator need**, not by guessing what to document. The apex rung is what makes the convergence *terminate* on hard cases: a disputed claim isn't stuck at `[GAP]` forever — one maintainer answer resolves it permanently for the whole pool. (Topic 10 §S42 demonstrated the loop end-to-end: gap-reaction fired 4/4 where a gap existed.)
...
- **`DELETE /property` S3 cleanup — the prospective gap-loop output (the headline demo).** The OpenAPI description claims *"Files are removed from S3,"* but the route (`entu/api routes/[db]/property/[_id]/index.delete.js`) and `utils/aggregate.js` only soft-delete the Mongo property + re-aggregate — **no S3 delete call exists in code** (mvox-dev's file-property-wire-shape note, Finn digest §2). Unlike JWT and date, **this one has *not* been filed as an Entu issue.** So it isn't a retrospective example of the loop — it is *exactly the PR the agent would file next*: a live, prospective gap-loop OUTPUT, sitting in the evidence right now, waiting for the first consult that touches file deletion. We can hand Argo the demo and the deliverable in the same breath. That is a stronger proof than the already-filed cases, because it shows the loop producing *new* signal, not re-narrating signal a human already produced. In schema terms it is the **code-beats-docs** case (§3a): the claim *"DELETE does not remove from S3"* is `src`-confirmed, the OpenAPI text `contradicts` it → code wins, so the claim is `backed`/`partial` (not disputed) and the OpenAPI desc is the gap to PR. The disagreement doesn't weaken the claim; it *names the doc bug.*

Everything in §1–§5 is the *productized* form of what this consult did by hand: the truth tables → backed competency claims (§4.1); Finn's OpenAPI check → the shared verifier step (§5); the docs PRs → the gap-loop flywheel (§3.4); the manual PO+lead+2-specialist coordination → the container + synergy map that lets the pool dispatch without re-engineering per consult.
...
1. **Where does `agents/` live?** `entu/api` (next to the code the claims cite)? `entu/www` (next to the docs the gap loop PRs)? A new dedicated repo (`entu/agents`)? Trade-off: co-locating with code keeps `type: src` evidence refs short and CI-checkable; a separate repo keeps the agent definitions from coupling to the API release cycle.
2. **Who owns index maintenance + PR triage?** The gap loop only compounds if someone merges the PRs. #42 itself names the issue/PR response asymmetry — the flywheel needs a committed triager. Is that Argo, a delegated maintainer, or the consultancy?
...
| §3 engagement protocol | ✅ | per-claim **derived** confidence label (stance rule); apex/disagreement/code-beats-docs cases; PR-not-issue gap loop with issue-only fallback |
