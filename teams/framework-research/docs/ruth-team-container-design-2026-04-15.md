...
| `TEAM_CONFIG_PATH` | Path within repo to team config dir | `teams/${TEAM_NAME}` | same | same |
...
| Inbox state (`teams/<team>/inboxes/`) | Separate team config repos (or separate paths within same repo) | `TEAM_CONFIG_PATH` parameter |
