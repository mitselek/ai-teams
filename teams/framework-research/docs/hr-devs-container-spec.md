...
| `apply-layout.sh` | `.mmp/hr-devs/teams/hr-devs/` | tmux layout creation script |
| `spawn_member.sh` | `.mmp/hr-devs/teams/hr-devs/` | Agent spawn script (adapted from hr-platform bare-metal version) |
...
The container uses exactly **one layout**: `full-review`, sourced from `hr-platform/teams/hr-devs/docs/tmux-layouts.md`. No other layouts (default, lite, full) are needed.
...
- Archive bare-metal memory files to `hr-platform/teams/hr-devs/memory/` (git commit)
