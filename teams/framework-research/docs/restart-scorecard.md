...
| # | Issue | Severity | Status |
|---|---|---|---|
| R4-1 | `$HOME` env var resolves to EMPTY STRING in some bash calls on Windows/Git Bash. Diagnose script checks `/teams/...` (root path) instead of correct user path. Other bash calls in same session resolve correctly. Shell initialization is inconsistent across bash invocations. | CRITICAL | OPEN — v4: replace `$HOME` with resolved absolute path; add validation gate |
