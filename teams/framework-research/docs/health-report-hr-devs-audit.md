...
`hr-platform/teams/hr-devs/memory/<name>.md`

But the reference files being shipped into the container have memory files at:
`reference/hr-devs/memory/<name>.md`

Per `hr-devs-container-spec.md §5`: "The roster.json, prompts/, common-prompt.md, and memory/ files remain in `hr-platform/teams/hr-devs/` (source of truth in the hr-platform repo). The container clones hr-platform — these files come in via git, not container bake."
...
`docs/startup-shutdown.md` (line 58+): "Team-lead reads `hr-platform/teams/hr-devs/apply-layout.sh`"
...
`~/github/hr-platform/teams/hr-devs/spawn_member.sh`
...
`~/github/dev-toolkit/teams/add-doc-to-dashboard.sh`

But in the container, workspace is `/home/ai-teams/workspace/`, not `~/github/`. The correct path would be:
`~/workspace/dev-toolkit/teams/add-doc-to-dashboard.sh`
...
|---|---|---|
| `apply-layout.sh` | `hr-platform/teams/hr-devs/` | **MISSING** |
| `spawn_member.sh` | `hr-platform/teams/hr-devs/` | **MISSING** |
| `startup.md` | `hr-platform/teams/hr-devs/` | **MISSING** |
...
`startup-shutdown.md` Step 0 line 4: "Read `hr-platform/teams/hr-devs/memory/lead.md` (scratchpad, if exists)"
