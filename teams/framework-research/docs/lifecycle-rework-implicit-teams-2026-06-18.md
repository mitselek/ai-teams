...
### The rotation contract has a teardown half: the restart MUST reap the prior courier's lock/orphan (*FR:Herald* -- 2026-06-18, S58)

The rotation fix (restart the courier at each session start so it re-resolves the live `session-<id>` -- startup Step 2.5) is the *acquire* half of courier-process rotation. There is a **teardown half this design did not state**, and a live S58 bug (Bug B: an orphaned prior-session courier + a stale lock surfaced at rotation) proved it must be explicit: **the restart MUST reap the PRIOR session's courier process and release its lock BEFORE the new courier acquires.** A rotation that re-resolves the path but leaves the old courier process running (still holding its lock, possibly still polling a dead dir) is incomplete -- the new courier either collides on the lock or two couriers race.

Until S58 this teardown lived ONLY in the `.ps1` wrapper (`stop-fr-courier.ps1`: hard-kill + external `--drain-once` + lock release, then `start`) and the separate lock-staleness spec (`docs/courier-lock-staleness-fix-spec-2026-06-15.md`, PID-1-starttime epoch for cross-boot stale-lock detection). Neither is the lifecycle *design*. **Contract statement, now folded in:** courier-process rotation is `reap-prior (kill + drain + release lock) -> resolve live dir -> acquire`. The reap is not optional and not merely a Windows `.ps1` implementation detail -- it is the symmetric half of the same rotation contract whose acquire half is "re-resolve on restart." The lock-staleness spec is the *mechanism* for detecting when a lock is reapable across a boot/PID-reuse boundary; this contract is the *requirement* that the reap happen at every rotation.
...
**Bare-session caveat (P6) -- CAVEATED by WS3b, see below:** on a brand-fresh session, `inboxes/` does not exist until first activity. Restore must `mkdir -p` the `inboxes/` dir (already in topic-06 Phase 4 "Why `mkdir -p` is required"). **Crucially, Step 3 does NOT depend on the P6 proactive-wake primitive at all:** restore is run by the session's OWN startup script while the session is **active** (mid-startup), not by an external writer into an idle session. It is a plain filesystem write followed by the session continuing its own sequence; the restored inboxes are read via the harness's normal **active-session delivery** path (the P4-class path -- members[]-routing + active-inbox-read, confirmed working on 2.1.181), NOT the idle-proactive-wake path (the P6-class path, inconclusive on 2.1.181). The two are distinct mechanisms; only the latter is in question, and Step 3 rides the former.

> **⚠ P6 re-test caveat (WS3b probe, 2.1.181, V5b):** the WS3b probe did **NOT** cleanly re-confirm P6 on the unpin-target version 2.1.181 -- an external inbox-write did NOT proactively wake a bare idle session within 35-90s (vs ~15s on S54/2.1.179). The result is **inconclusive-leaning-negative** with a **named confound**: the probe drove the session purely headless (`docker exec` + `tmux send-keys`, never an attached pane; `focus-events off`), and the proactive-wake may depend on a terminal-activity/focus signal a never-attached pane doesn't generate. **Impact on this step:** Step 3 restore's correctness does NOT depend on *proactive* wake -- restore runs *during* startup (the session is active, not idle-waiting), so `mkdir -p` + file-write + the existing restore script are unaffected. The P6 question bites the **steady-state inbound-while-idle** path (a courier writing to a quiescent session's inbox and expecting a proactive wake), which is the [teamless-courier RfC](rfc-teamless-courier-2026-06-17.md) headline, not this restore step. **Recommendation:** re-test P6 with an *attached* pane on 2.1.181 before any design (RfC async-proactive-wake default, or a courier relying on idle-session wake) leans on it; if it's a real 2.1.181 regression, the fallback is the courier's own poll loop. Restore (Step 3) is safe either way. **Disposition (Aen, 2026-06-18):** V5b/P6 is **NON-BLOCKING for the unpin** -- Brunel confirmed the courier polls its outbox/inbox on its loop interval, so cross-team delivery is guaranteed regardless of proactive-wake. P6 is therefore a *latency* property (does a quiescent session wake *immediately* on an external write, vs. on its next poll), not a *delivery* property. The attached-pane re-test is deferred RfC-scope (a separate task), not an unpin gate; this Step 3 and cross-team delivery both hold without it.
...
- **Remediation (out-of-band, NOT a mandatory lifecycle step) -- PROMOTED by the V3 finding:** an optional startup-time or cron sweep that removes `~/.claude/teams/session-*/` dirs **not** backed by a live session. The WS3b probe (V3) **promotes this from "convenience" to "recommended"**: because `sessions/<pid>.json` lingers `status:"idle"` for dead sessions (NOT GC'd), stale dirs accumulate AND the resolver's liveness disambiguation depends entirely on **process-liveness** (not status) -- so a `_pid_alive`-keyed sweep is the clean GC the substrate doesn't provide. **Hazard to encode:** the sweep MUST exclude the live session's own dir and key liveness on **process-liveness** (`_pid_alive` on the entry's `pid`, optionally `procStart`-guarded), NOT on the `status` field (V3: `status` is useless -- dead lingers `idle`). It stays OUT of the mandatory startup/shutdown sequence so a failed sweep never blocks a session. Cataloged as a Callimachus gotcha (`no-teamdelete-stale-session-dirs-accumulate`, confidence to bump medium->high now that V3 confirmed the GC behavior).
...
| Multi-team **named** host (pre-unpin 2.1.177) -- bare glob is ambiguous | The current dev host has N named team dirs (`framework-research`, `cloudflare-builders`, ...). On 2.1.177 the dirs are *named*, but the liveness filter matches `session-<sessionId[:8]>` against the dir name -- a **named** dir can never match the `session-<hex>` pattern, so the liveness filter returns `live: []` and the resolver FAILs FAST. **Empirically confirmed on the live box 2026-06-18** (`--resolve-team-dir` over 12 named dirs -> `ambiguous ... (live: [])` rc=1). | Correct fail-fast (never guesses among N teams). The **explicit override is the pre-unpin disambiguator**: `--team-dir-name framework-research` -> resolves rc=0 (verified). **Invocation contract for the lifecycle scripts:** pre-unpin (named multi-team host) pass `--team-dir-name framework-research`; post-unpin (2.1.178+ `session-<id>` dirs) the in-session caller passes `--session-pid $$` so the pid tiebreaker resolves cleanly even on a multi-team host. Bare `--resolve-team-dir` is only safe on a single-team host. This is why the glob-liveness path is **designed for the 2.1.178+ `session-<id>` world**; on 2.1.177 named teams, discovery is via the override (equivalent to the old hardcoded path, routed through the resolver). |
...
2. **[RESOLVED -- WORST CASE -- WS3b probe, 2.1.181, V3+V4]** `sessions/<pid>.json` GC behavior + write-order (shared with Brunel's OQ3). **GC (V3):** the dead entry is NOT GC'd on exit (graceful AND kill -9) and **lingers with `status:"idle"` -- identical to a live idle session.** `status` CANNOT distinguish dead from live; there is no dead/exited/stopped status. So the resolver's status-allowlist liveness was **broken** (V2b fail-fast'd on a dead pid-344). **Fix (must-land before unpin):** liveness keys on **process-liveness** (the existing cross-platform `_pid_alive(pid)` in `stationmaster-courier.py` -- Windows tasklist + POSIX os.kill, conservative-on-error -- NOT a status string, NOT a Linux-only replacement), optionally `procStart`-guarded against PID reuse (the field IS in the substrate; `/proc/<pid>/stat` field-22, Linux-only add-on that degrades on Windows). **Write-order (V4):** `config.json` first, `sessions/<pid>.json` ~10-25s later at interactive-ready -> the early-discovery window is real; Step 2' prefers wait-for-pid (folded into §4.2). Integration: Brunel sends `resolve_team_dir`/`discover_by_*` (new, clean); Herald owns the `_pid_alive` wiring + procStart layer (collision-avoidance with the existing helper). The stale-dir sweep (OQ3/§6) is **PROMOTED** by V3 -- with status-liveness broken, a pid-liveness sweep is the clean GC.
...
6. **[OPEN -- v2/at-scale, NOT a v1 blocker -- *FR:Herald* 2026-06-18, S58]** **Single-point (Config-load) inbox-dir resolution is not self-healing.** The rotation fix resolves `inboxes_dir` ONCE, at courier Config-load, and refreshes it only by a per-session **restart** (startup Step 2.5). A live S58 bug (Bug C: a stale explicit `inboxes_dir` resolved at a prior Config-load survived into a session where the dir was dead) is the v1 instance of this: it confirms the once-at-load resolution is the root hazard, and that restart-to-refresh is the *only* refresh path. **For v1 (FR sole live 2.1.178+ team) this is correct and sufficient** -- the per-session restart cadence matches the per-session team-dir rotation cadence, so there is no window where the live dir rotates without a restart. **It does NOT generalize.** The contract has no *mid-session* re-validation: if the live session's dir rotates or dies WITHOUT a courier restart -- e.g. a session crash + re-derive, or the **v2 multi-migrated-team** case where bare liveness sees >1 live `session-<id>` and the courier's bound dir can change under it -- a once-resolved courier silently delivers nothing, with no detection. **At v2/N-teams the discovery contract needs a self-healing layer:** either (a) periodic re-resolution of `inboxes_dir` on the courier's poll loop, or (b) a liveness check on the resolved `inboxes_dir` before each delivery (re-resolve on stale). Both turn "single-point resolution + restart-to-refresh" into "continuous resolution." **Disposition:** flagged as a v2/at-scale OPEN here; **explicitly NOT a v1 unpin blocker** (Aen, 2026-06-18). Owner TBD when a second team migrates (likely the courier/WS1 surface, since it is a poll-loop/per-delivery change). Pairs with the courier-process rotation teardown contract (Section 3) -- both are the rotation contract's at-scale hardening.
...
- **GOTCHA** `gotchas/cold-start-window-false-negative` (*FR:Herald* -- S58): within ~25s of a 2.1.178+ cold start, `config.json` exists (eager) but `sessions/<pid>.json` does NOT yet (V4 write-order). A discovery/liveness/existence probe run in this window can mis-read the partial state as "lazy-create, nothing here" and falsely deny the eager-write property -- S57's halt was exactly this false negative. Rule: AWAIT/RETRY before reporting absence within the window; never conclude lazy-create. Links [[teams-substrate-2.1.179-implicit-teams]], [[startup-create-collapses-to-discover]].
- **CONTRACT** `contracts/courier-rotation-reap-prior` (*FR:Herald* -- S58): courier-process rotation is `reap-prior (kill + drain + release lock) -> resolve live dir -> acquire`. The reap (teardown half) is not optional and not a Windows-`.ps1`-only detail; it is the symmetric half of "re-resolve on restart." Bug B (orphan courier + stale lock at rotation) is the failure when the reap is skipped. The lock-staleness spec is the reapability *mechanism*; this contract is the *requirement*. Links [[courier-must-runtime-discover-team-name]].
- **OPEN/RfC** `at-scale single-point inbox-dir resolution is not self-healing` (*FR:Herald* -- S58, v2 only): once-at-Config-load resolution + per-session restart-to-refresh is correct for v1 (single live team) but has no mid-session re-validation; at v2/N-teams (or session crash + re-derive) a once-resolved courier silently delivers nothing. Bug C is the v1 instance. v2 fix: poll-loop re-resolution OR per-delivery `inboxes_dir` liveness check. NOT a v1 unpin blocker. (File when v2 work opens.)
