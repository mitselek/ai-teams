...
4. **Run order:** V3 FIRST (tells us whether V2b's liveness path is load-bearing), then V4 (cold-start measurement), then V1 (courier glob), then **V5 (P4+P6 regression -- reuses V1's live session + config)**, then V2 (lifecycle pid + planted stale dir; runs LAST so its planted `session-deadbeef` dir can't confuse V5's P4/P6 checks). Full order: **V3 -> V4 -> V1 -> V5 -> V2.** V3-before-V2 is deliberate (V3's GC finding determines how to read V2b).
   ...
   > **CORRECTION (post-probe, 2026-06-18):** this order CONTAMINATED the in-sequence verdicts -- V3's own finding is that `sessions/<pid>.json` does NOT GC, so V3/V4's spawned-victim sessions linger and poison the resolver state V1/V5/V2 read (their on-screen verdicts were artifacts; true verdicts came from Hopper's clean direct re-probes). The harness drive order is **corrected to `V1 -> V5 -> V4 -> V3 -> V2`**: clean-state resolver checks (V1, V5) FIRST, then the session-spawners (V4, V3), then V2 LAST with its own controlled plant+cleanup. V3 no longer needs to run first to inform V2b -- its GC finding is now baked into the process-liveness resolver fix. The re-run uses this corrected order.
