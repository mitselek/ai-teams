...
Audit: `grep -rl 'stage-2: pending' patterns/cards/`.

**[GATE RELABEL 2026-08-19]** The pre-gate backlog in this subdir has been relabelled `stage-2: pending` -> **`stage-2: legacy-unaudited`** (32 cards). Those entries were filed BEFORE the 2026-06-02 Stage-2 gate existed and were stamped `pending` by the fail-closed default -- but `pending` means *a named co-author owes a read-back*, and for a pre-gate entry nobody does. Keeping them under the same label made `grep -rl 'stage-2: pending'` return 49 hits against 10 real obligations wiki-wide. **A fail-closed default applied retroactively manufactures a backlog indistinguishable from real obligation.** `legacy-unaudited` is not a to-do list -- it records that nobody checked, and that nobody is owed. Four-state table + ruling: [`../../process/stage-2-confirms-filing-gate.md`](../../process/stage-2-confirms-filing-gate.md). Current patterns/ gate: **confirmed 56 / partial 3 / pending 2 / legacy-unaudited 32**. (*FR:Callimachus*)
