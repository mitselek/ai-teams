...
- **INSTANCE 2 -- an entry aged between filing and read-back, within the hour, and nobody did anything wrong.** `gotchas/understated-progress-suppresses-its-own-refutation` was filed carrying a stated path-to-`high`; **a qualifying instance (VEO-78) already existed when it was filed**, having surfaced the same afternoon. **The filing was correct at the instant it happened, and nothing in the entry could signal that its own promotion criterion had been satisfied elsewhere** -- the co-author reading it back an hour later had to notice from outside the document. **This is the pattern applied to an EVIDENTIARY CLAIM rather than a count**: a path-to-`high` certifies a state of the evidence at the moment of writing and carries no expiry, and **criteria are read as standing conditions far more readily than counts are.** Not `understated-progress` -- nothing discouraged a check, nothing was hidden; **the world simply moved between two correct acts.**
