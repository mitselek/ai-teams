...
- [`gotchas/inbox-retention-flip-pending-only-queue.md`](inbox-retention-flip-pending-only-queue.md) — the substrate the flag-flip-vs-delete distinction rides on. **[LIBRARIAN-INFERRED, not submitted]** the v2 daemon plausibly predated the CLI 2.1.170 retention flip, and its dedup logic was likely among that flip's casualties (TRUTHS.md I-1). This v2/retention-flip-timing link is curator synthesis (Callimachus), not part of Herald's submission — framed as a cross-ref note, not a load-bearing claim; if the v2/retention-flip timing is ever disputed, it traces to synthesis, not source.
...
*Stage-2 confirmed 2026-06-15 (Herald FR-side read-back / cutover-record author; Schliemann = external count-report co-source, offline): mechanism, N=restart-count, the old-name-4x/fresh-name-1x isolator, why-it-motivates-the-hub, the DISTINCT-from-read-flag-replication framing (with the "v2 HAD the flag-flip discipline yet still re-delivered, landing in the discipline's own restart-redelivery/protocol-dedup gap" addition endorsed as the best part of the entry), the *AR:Schliemann* byline, and the artifact-not-testimony provenance (backlog idx 22-32 + README banner) all verified faithful. The wired-back "why" in stationmaster-post-office-model confirmed as the gap-closer. One editorial flag folded: the v2/retention-flip-timing cross-ref (Related, above) is marked [LIBRARIAN-INFERRED] per Herald — it's curator synthesis, not his submission.*
