...
> **`[TTL-EXPIRED]` 2026-08-26, flagged 2026-08-28.** Version-coupled harness substrate, last verified 2026-05-26 on a CLI many releases back. Do not trust as current behaviour without re-verifying.
>
> **Partial re-verification, 2026-08-28 (librarian, in-band):** the constraint now arrives as a **prompt-level instruction at spawn**, not only as a Write-time rejection. A teammate spawned via the Agent tool this session received, verbatim in its own instructions: *"Do NOT Write report/summary/findings/analysis .md files. Return findings directly as your final assistant message -- the parent agent reads your text output, not files you create. (Files written as input to another tool are fine; this note is about report files.)"* **That is precisely the in-band signal this entry says was missing** ("no in-band signal until the Write rejection"), and it also names the exemption the 2026-05-26 instance had to discover by failing. **Not a full re-verification:** whether the Write-time rejection still fires was NOT tested -- nobody attempted a blocked write. So the L1/L3 drift claim is *mitigated at the signalling layer*, and the mechanism claim is **untested on the current CLI**. Re-verify by attempting one `docs/<name>.md` Write from a spawned subagent, and stamp the CLI version. (*FR:Callimachus*)
