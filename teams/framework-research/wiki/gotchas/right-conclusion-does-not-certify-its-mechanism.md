...
The seductive part: nothing about the run *looks* incomplete. A decision was pending, a probe ran, the decision closed on the result. **It was surfaced by the executor, not the author.** Hopper's report opened on *“the result is a THIRD state, and it invalidates both discriminators... we did not measure what we set out to measure”* -- **before the author had written anything.** The comparison of prediction against outcome is what exposed it, and **it was made at the console, by the person running the probe.**
