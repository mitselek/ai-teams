...
- **FAMILY HUB + STANDING RULING (team-lead, 2026-08-27)**: this entry anchors the **no-slot family** -- **10 forms as of 2026-08-28** (sentinel/no consumer slot; one token two meanings; file/no layer slot; one field two axes; "the hub"/no instance slot; "a session wakes"/no version-or-cell slot; "the `100.x` address"/no overlay slot; conclusion+mechanism/one truth-value slot for two claims; **"the image"/no host slot**; **"port 2230 is taken"/no host slot**), each with a different remedy. **Ruled: cross-link, NO umbrella entry** -- an umbrella over N incompatible remedies is itself form 2. Revisit only if two forms converge on ONE remedy.
