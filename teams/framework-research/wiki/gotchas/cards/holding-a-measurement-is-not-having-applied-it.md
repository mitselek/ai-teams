...
- **INSTANCE C -- sub-shape B with a PRINCIPLE instead of a number (Finn, self-reported).** He argued a `stage-2: confirmed` was correct by invoking team-lead's own finding that *`confirmed` certifies a co-author READ the entry, not that it is correct.* **The principle is true; it does not govern the question**, which was whether a self-report predating the entry can be a read-back -- **procedure, not semantics.** **He reached for the sharpest tool nearby rather than the one that fit.** **This instance exists to stop the entry being read as being about arithmetic**: both original instances are numeric, and **an entry citing only arithmetic gets filed under numeracy and its subject missed.** The subject is **the relation between an instrument and a question, and that relation is substrate-independent** -- a principle, lemma, benchmark, precedent, or test result can each be correct, in hand, and applied to something it does not answer.
...
- **Confidence medium, submitter declined to be talked up.** Three instances, one session, one team, **all authored by the two agents who then diagnosed them** (**instance C does nothing for that limit** -- same submitter -- and was added for **coverage of the genus**, not weight) — same correlation limit as `understated-progress...` and `roster-drift...`. **Path to high**: another team, or **an instance where the number-holder is not the person who spots the mismatch.**
