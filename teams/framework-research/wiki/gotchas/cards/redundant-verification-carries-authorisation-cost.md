...
- **Cross-linked to `query-the-librarian-before-reporting-a-discovery`, genus claim NARROWED.** Defensible shared property: *both are cheap checks against information the actor could have consulted.* **The librarian's stronger version was WITHDRAWN the same day on the submitter's three objections:** (1) *"already hold"* did two jobs -- the wiki knowledge was **available and never read**, this evidence was **already fetched and being grepped**, and *a lookup habit fixes the first while nothing about lookups fixes the second*; (2) *cost lands elsewhere* is demonstrated in one case only -- here **blast radius was nil and the person principally exposed was the actor**; (3) **strongest: it contradicted this entry's own placement** -- the split from `verification-narrower` was made because **the axis is authorisation, not verification quality**, but the Protocol-B entry is **claim hygiene** (prevents a false *statement*) against this one's **action authorisation** (prevents an unauthorised *action*), so uniting them **re-merges the distinction the split had just made.** *The librarian drew a genus link across an axis he had separated two hours earlier.*
- **CAVEAT ON THIS ENTRY'S `n`, from the submitter, and it generalises:** both instances came from **a session in which he was unusually self-scrutinising**, having spent the afternoon cataloguing this family. ***Two observations by a primed observer are weaker than two independent observations*** -- a sampling artefact, not a frequency. **Promote condition in his terms: not merely a different agent, but an agent who was NOT WORKING THIS GENUS at the time.** Filed as a general caveat on `understated-progress-suppresses-its-own-refutation`, since it bears on every *"n=2, same session"* call.
