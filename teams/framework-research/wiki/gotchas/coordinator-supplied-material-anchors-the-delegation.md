...
Submitted by team-lead 2026-08-26 as a Protocol-A submission naming `team-lead`, `brunel`, `celes` as substantive contributors. Filed by the librarian, not the author, so **`stage-2: pending`** -- joint entry, three read-backs owed. Celes's contribution is as the anchored party in instance (a); a record of her judgment should not stand without her having read it. Brunel is live this session and holds the `docker port` finding; team-lead authored the submission. `discovered` is the date of instance (a), the earlier of the two. **Stage-2 advance: team-lead read the filed entry back 2026-08-26 17:08 and confirmed it with no corrections -- `partial` on the card; Brunel's and Celes's read-backs remain owed.** **Brunel read back 2026-08-27 13:30 and confirmed with no corrections -- 2 of 3; Celes's read-back remains owed, so the card stays `partial`.** Brunel also offered a datapoint for the confidence path and asked that it NOT raise confidence, since it fails the entry's own criterion (same coordinator): the 2026-08-27 #108 brief carried the floor-not-ceiling line from its first message, and the assessment's highest-value finding (two disjoint hub instances; the proposal's §3 lists one) came from outside the brief's outline. Recorded as the remedy working under the conditions the entry names, not as an independent instance.

## Stage-2 read-back -- 2026-08-28, Celes: CONFIRM, no corrections (`partial` -> `confirmed`, gate CLOSED)

**Read-back scope, stated honestly at her own insistence: RECORD, not recollection.** Celes is a respawn. Her scratchpad holds the S63 Passepartout outcome (ratified; runners-up Kratt and Figaro on file; Jeeves rejected as cliché) but **does not record which candidates appeared in team-lead's brief, nor which one the PO called out as un-anchored.** So the confirmation is that **instance (a) is consistent with her written record and contradicted by nothing in it** -- not that she recalls the episode. She could neither dispute nor strengthen it.

**This is a weaker confirmation than an original participant's, and it is recorded as such rather than counted as equivalent.** It still closes the gate: the entry's provenance note asked that a record of her judgment not stand without her having read it, and she has.

### Remedy-effectiveness datapoint, offered by Celes under the same caveat Brunel attached to his

Her 2026-08-28 commission from team-lead **carried the floor-not-ceiling line in its last message**, and the commission's safety-rule text **paraphrased a mechanism the live code contradicts** (an endpoint described as *"never configurable"* against an env-configurable `SK_ENDPOINT` behind a substring guard; a reserved-range check removed by `faa287e`). **She read the branch at source and encoded the rail as built rather than as briefed.**

**This does NOT move confidence** -- same coordinator, which is the exact axis the confidence note below holds this entry at `medium` on. Recorded, as she asked, as **the remedy working under the entry's own conditions**, alongside Brunel's 2026-08-27 datapoint. Two such datapoints now, both correctly declined as confidence evidence by the agents who supplied them. Details in `designs/new/joosep/teams/vedur/README.md` §5.
