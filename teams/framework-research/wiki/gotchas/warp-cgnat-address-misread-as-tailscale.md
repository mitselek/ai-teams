...
The mechanism of the misread: **a range is a plausible-feeling discriminator that was never a discriminator.** This is [`../patterns/discriminator-anchored-on-sub-canonical-source.md`](../patterns/discriminator-anchored-on-sub-canonical-source.md) Sub-shape A.1 with an IP prefix as the identifier -- the author anchors on a remembered convention ("100.x means Tailscale") instead of the canonical source (what the host's own overlay client reports). It is also a **no-slot** form: *"the `100.x` address"* has no slot for **which overlay**, and each writer fills it with the overlay they know. See the family note in [`file-state-claims-have-no-layer-dimension.md`](file-state-claims-have-no-layer-dimension.md).
