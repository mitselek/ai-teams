...
**Form 12 added 2026-09-02 (Hopper, Brunel second source):** *"the operation's tier"* with no slot for **whether the change is declared in the substrate's source of truth** ([`../contracts/tier-taxonomy-has-no-slot-for-undeclared-in-place-change.md`](../contracts/tier-taxonomy-has-no-slot-for-undeclared-in-place-change.md)). An in-place `docker exec ... npm install -g` is not Tier R (it mutates), not Tier M (no substrate script handles it, because none knows it happened) and not Tier D (no state dropped, no irreversible-data-loss surface, stated rollback). **The R/M/D tiers grade risk at the moment of execution; the missing dimension is durability**, and it is missing because the taxonomy never had a reason to measure it. **Sibling of form 11** -- both are gaps in an *authorization schema*, and both were found by an agent surfacing the missing dimension rather than picking the nearest label. **Its own remedy, per the standing ruling:** the cost is not at execution (Tier M sanction suffices) but **in the reporting** -- an undeclared in-place change must never be reported as done until the durable path lands. Cross-link, no umbrella.
