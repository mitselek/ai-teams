...
1. **Our own team is the counter-example.** `teams/framework-research/roster.json` pins `claude-fable-5[1m]` for all 10 slots. The team ran **off-pin on Opus 5 for three consecutive sessions through 2026-08-12**, and back **on-pin on 2026-08-19** — while the roster file **never changed in either direction.** That is the sharper version of the point: the field was equally "correct" whether or not it matched what was running. **A field that does not move when reality moves is not a record of reality.**
