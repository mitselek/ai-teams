...
- **DESIGN-REPO ≠ DEPLOYED, three independent reasons.** (1) **Our own team is the counter-example**: roster pins `fable-5[1m]` for all 10 slots; the team ran **off-pin on Opus 5 for three consecutive sessions through 2026-08-12** and **back on-pin 2026-08-19** — while the roster file **never changed in either direction**. **A field that does not move when reality moves is not a record of reality.** (2) `roster.json`'s own `_substrate_note`: on Agent-tool architecture the `model` field is **documentation-only**, TeamCreate stamps the parent session model regardless. (3) **Coverage gap**: of 7 `designs/deployed/` dirs only 5 carry model data, 2 in non-standard files that **`find -name roster.json` misses** — which is how the original survey ran.
