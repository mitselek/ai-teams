...
| Implicit-Teams Substrate Datapoint -- CLI 2.1.258 | [card](teams-substrate-2.1.258-implicit-teams.md) · [full](../teams-substrate-2.1.258-implicit-teams.md) |
...
> **[+1 CARD 2026-09-02, S71]** `teams-substrate-2.1.258-implicit-teams` (callimachus rows 1-3, team-lead row 4) = **pending**, mixed authorship, fail-closed; team-lead read-back advances it. Header 12 -> 13, counted against files on disk. **Gate line corrected 10/0 -> 10/3**: it had never been updated for the two 2026-08-31 2.1.251 sheets, both filed `pending`. **Retire-and-forward held** -- a version-pointer sheet, not a fold and not a fresh full sheet; **its NOT-re-measured table is the load-bearing half**, and the `sessions/<pid>.json` GC row is recorded as a census that cannot attribute kill type, precisely so it is not later read as a measurement. TTL aligned to 2026-11-30 with its two siblings so the lineage re-verifies in one pass. (*FR:Callimachus*)

> **[GATE ADVANCED 2026-09-02]** `teams-substrate-2.1.258-implicit-teams` reached **`confirmed`** on team-lead's read-back of row 4. Measured: **confirmed 11 / partial 0 / pending 2 = 13.** **Note the shape of that entry, which is instructive rather than anomalous: it is `stage-2: confirmed` AND carries a DISPUTED row.** The read-back certifies the row faithfully records what was reported; the dispute records that the substrate claim was later falsified by measurement. **`confirmed` certifies a procedure, not correctness.** (*FR:Callimachus*)
