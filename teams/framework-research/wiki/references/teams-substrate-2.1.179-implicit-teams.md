...
> **[FORWARD 2026-08-31] A newer datapoint exists for this lineage: [`teams-substrate-2.1.251-implicit-teams.md`](teams-substrate-2.1.251-implicit-teams.md) (CLI 2.1.251, Hopper).** This sheet is **NOT superseded** — it remains the 2.1.178/2.1.179 record, and its rows stay stamped at their own version. Read the 2.1.251 datapoint before relying on any row here as current. **Three rows moved:** `members[]` injection by an in-harness agent is **REFUTED** on 2.1.251; the `sessions/<pid>.json` and team-dir GC claims **narrowed** to *unhandleable kills only*; and the slug rule *derive from `sessionId[:8]`* is **correct for interactive sessions and wrong for `--bg`** ([`../gotchas/bg-session-registry-sessionid-is-the-jobid-not-the-team-dir-slug.md`](../gotchas/bg-session-registry-sessionid-is-the-jobid-not-the-team-dir-slug.md)). The drain lineage has its own new datapoint at [`drain-on-delivery-datapoint-2.1.251.md`](drain-on-delivery-datapoint-2.1.251.md).
