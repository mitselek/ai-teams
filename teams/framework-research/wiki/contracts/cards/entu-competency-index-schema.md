...
- **One-claim-per-file** → diffable, PR-able (gap loop emits atomic PRs — matches #42 "PRs land, issues stall"), auditable-before-hiring (`grep confidence: unverified` shows where the agent is honest).
...
- **`maintainer-authoritative` = apex + Argo escalation-records-back (§2b).** Gap-loop is a 3-rung ladder: **PR** (doc text agent can write) → **issue** (maintainer-only change) → **escalate-to-Argo** (question only maintainer answers, e.g. JWT 12h/48h). Argo's answer comes back as apex evidence → claim apex-backed for the whole pool. Strongest form of #42's "Argo gets prioritized signal" — scarce attention captured once per gap.
