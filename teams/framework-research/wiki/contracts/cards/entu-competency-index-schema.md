...
- **§3 ref-formats GROUNDED (S44) then CORRECTED (2026-08-31).** Real shapes: `src`→`entu/api: utils/*.js (symbol)` (Nitro file-router, NOT `src/api/*`); `docs`→`entu/www: src/api/*#anchor` or `PR #N`; `openapi`→`https://api.entu.app/openapi` tag+op; `probe`→`mvox-dev: docs/migration/findings/<note>-YYYY-MM-DD` (**short prefix + mandatory date**; probe-script + result-JSON + ratified-commit + STEP\|OP\|RESULT table maps 1:1). Three real doc↔code discrepancies (JWT 12h/48h, date wire-format, S3-cleanup) are the gap-loop's worked examples; `src`>`docs` precedence where they conflict.

- **TTL fired 2026-08-31 and found NO re-drift -- but four born-wrong exemplars** (Finn, Group 4). Substrate stable across 3 months; `openapi` + `src` rows clean end-to-end. Corrected: `probe` prefix/date (the row disagreed with its own example; the dateless file has **zero commits ever**), `docs` anchor `#single-hop`→`#field-references` (**not a rename** -- byte-identical heading set at the S44 sha), a non-verbatim §1 excerpt citing `_parent.*` (**0 occurrences**, and the doc says `_parent` is *not* counted by `_referrer`), and a **PR-form ref that was unfalsifiable** -- kept, with its own rule (records merge state; stale on merge-state change or close-unmerged; unmerged = not yet authoritative). Rejected alternative: dropping the PR form, which would make our own gap-loop output uncitable. **Root cause is the filer's:** `source-files` named the grounding digest, which has **zero** `ref:` lines -- the 32 exemplars live in the sibling **competency harvest**, now added. Generalised at [`gotchas/citation-names-the-wrong-sibling-source.md`](../../gotchas/citation-names-the-wrong-sibling-source.md). Watches: Entu shipped its own AI assistant (~2026-07) = candidate 5th domain, **held as watch, roster unchanged**; §6a's "timing is Entu's" now has a datapoint (11 issues open, PRs #11/#13 unmerged ~3 months).
