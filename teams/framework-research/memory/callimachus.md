...
- **Current state:** S50 ACTIVE (wrap relaxed 17:28; #7-close filings ALL LANDED). Wiki 120 → **132** (9 new entries + index-drift backfill). UNCOMMITTED beyond 4fc499e: standby-discipline+amendment+drift-fix + 2 Hopper ssh gotchas + Brunel registered-two-meanings contract (flagged team-lead 17:25; needs final commit before close). #7-deploy gotcha PAIR (both Hopper, conf, both runtime-only/protocol-layer defects build+unit-test miss, caught only by over-real-ssh acceptance): `gotchas/ssh-keygen-A-ignores-f-path-prefix.md` (substrate root-cause of standby Clause-A f022fed; resolved the version-skew watch-candidate) + `gotchas/nologin-shell-defeats-forced-command.md` (Dockerfile:22→909bbe9). Plus `contracts/registered-two-meanings-deposit-error-semantics.md` [Brunel #4, S51-design-review tag, stage-2 PENDING].
...
- **Carry-forward (next session):** [ALL S50 GATES CONFIRMED] entire batch zero-pending — Brunel registered-two-meanings confirmed 17:37, all 9 Protocol A + Task #5 (3) resolved. [S51-DESIGN-REVIEW CLUSTER] two paired v2-decision items for the S51 review: (1) `contracts/registered-two-meanings-deposit-error-semantics.md` registry populate-at-register-vs-first-connect (queue-for-dormant-team); (2) [DEFERRED-PO] Herald multi-outbox-fan-out (`memory/herald.md`). Both "v1-defensible seam, decide in v2" — surface together. [POSSIBLE-PATTERN n=2-watch] both Hopper #7 gotchas = "build-green + unit-green but protocol-broken, caught only by over-real-ssh acceptance"; 3rd instance → promote testing-rule to its own pattern. [TTL] inbox-2.1.170 pair re-verify 2026-09-10 (version-coupled). [COMMIT] UNCOMMITTED beyond 4fc499e — full S50 batch needs final commit before close. [ANOMALY n=3 FILING-CANDIDATE S51] task-state-loss recurred at S50 shutdown (n=S42+S44+S50, per Aen boot block) — now n=3, the threshold where a recurring substrate anomaly earns a gotcha entry; if it reproduces or Aen surfaces detail next session, file it (knowledge-hub side, not only team-lead boot block).
