...
- **Active items:** Schema contract FROZEN at HEAD (team-lead freeze call — no micro-edits). Now CONSUMED: gave Celes schema-conformance SIGN-OFF on data-lifecycle `competencies.yaml` (Task #4) — Finn's 18 claims (Task #5 harvest) all §1/§2a/§3a-clean, fold verbatim; specified the worked-draft provenance-header shape (`source-commit: null` + `generation-status: worked-draft` = machine-visible-not-just-comment) + derived roll-up. Conformance-check ≠ contract edit, so clear of freeze. Protocol-A pattern `citation-backed-beats-posture-backed`: **stage-2:PARTIAL** (advances on **Finn** read-back). Open watches: Finn's read-back + any further competencies.yaml conformance checks (auth/formula/schema domains) as Celes assembles them.
...
[DECISION] PO-approved schema v2 extension (S44 directive): (a) 5-rung method ladder — doc-cited/spec-derived/live-probe/**live-audit**/**maintainer-authoritative**, ordered by authority; live-audit=probe-over-real-prod-data (prevalence claims, NOT a new type — still type:probe); maintainer-authoritative=APEX (Argo direct answer outranks all). (b) §2b Argo escalation-records-back: Argo's answer recorded as apex evidence → claim apex-backed for whole pool. (c) §3a `stance` (confirms/contradicts/supersedes) per evidence + derived-confidence rule: apex-override / supersede-drop / equal-rank-deadlock→disputed+[GAP] / **code-beats-docs exception** (src-confirm vs docs-contradict = claim holds, docs is gap-target). JWT 12h/48h = canonical apex/escalate (code-vs-code deadlock); S3-delete + date-format = code-beats-docs. Re-points WikiProvenance whole-entry dispute model to per-evidence granularity.

[DECISION→SUPERSEDES] PO gap-loop revision (S44, AFTER v2): the gap-loop is a **signal producer, NOT a remediation engine** — do NOT design PR-preference to route around Entu's broken issue-triage (that's engineering around Argo's pipeline). KILLED the 3-rung PR→issue→escalate ladder. Replaced with TWO distinct mechanisms (§6): (a) **gap reporting** — missing/wrong doc → structured evidence-backed report, **issue-DEFAULT**, PR a channel-neutral option (NOT preferred), explicit no-PR-bias; (b) **dispute resolution** — contradicting-evidence-no-code-settles → escalate to Argo → maintainer-authoritative apex evidence (UNCHANGED, it's evidence-authority not doc-routing). Removed all "PRs land/issues stall/optimised for channel that works" framing (§2b/§4/§6); §4 one-claim-per-file reframed as index-INTERNAL change hygiene, decoupled from Entu reporting channel. [PATTERN] when designing a feedback loop that hands signal to an external party, scope its responsibility to "emit a high-quality signal" — NOT to "ensure the external party acts"; baking the external party's pipeline-weakness into your design = engineering around their dysfunction, an anti-pattern.
