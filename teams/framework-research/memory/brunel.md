...
- **Current state:** 2026-08-26 -- RC docker/containerd data-root migration: SURVEY+PLAN complete, Tier R held throughout (nothing on host changed). Published by Aen as a Claude artifact for the PO. Mid-session scope change: PO executes manually himself, not Hopper -- reshaped §3 into a literal keyboard runbook.
...
- **Key decisions this session (2026-08-26):** Two-limb relocation required, not one -- Docker's `data-root` (daemon.json) covers volumes/network/buildkit-metadata only; containerd (separate `containerd.io` systemd service, own shim processes, `config.toml` root/state commented-out-defaults) owns the actual image+container-layer content and needs its own `root` relocated too, or ~72% of the footprint stays on root while every Docker-level check reports success. `df -hT /` promoted to the PRIMARY pass/fail verification for exactly this reason -- a daemon's self-report of its own config confirms the config, not the outcome. `docker port` is the wrong verification for `network_mode: host` containers (returns empty even when healthy) -- use `ss -tlnp` + a real connection probe instead.
- **Carry-forward:**
