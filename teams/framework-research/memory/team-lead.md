...
- **Current state:** S71 (2026-09-02 15:52-~19:35 wall clock; agent-reported stamps in this file and the ops-log run up to 2 h behind the wall clock) CLOSED clean. Parent Fable 5.1, CLI 2.1.258, SLUG `session-1e8d8ae9`, pid 26376. Cell = Brunel + Hopper + Callimachus + Volta, all spawned `model: "opus"` on PO order; all four shut down cleanly.
...
- **Wiki:** 229 -> 235 entries (Cal); roster-drift entry `status: disputed` -- do NOT cite config.json `model` values (see WARNING below). Three Protocol C candidates open (boot item 6).
...
- **[LEARNED, lifecycle] A terminated agent is RESUMED by any SendMessage to it.** After all four `teammate_terminated`, Cal and Brunel resumed each other for ~40 min of read-back/filing exchange (real output: entry 235, pronoun sweep scope, stage-2 referent finding) that I never asked for and could not stop without sending more messages. `teammate_terminated` is not final; the shutdown_approved is consumed and a fresh shutdown_request is needed. Volta item: shutdown protocol must end with a no-further-sends rule in the request text, and S4 persist must follow the LAST termination, not the first round.
...
6. **If PO raises prompts/roster/schema -- THREE Protocol C candidates, none decided:** (a) `prompts/hopper.md` tier taxonomy: 4th class vs orthogonal "represented in source of truth" flag (Cal reads flag as better); Celes owns prompts. (b) the unresolved `resolved` enum (G3, mine). (c) wiki `stage-2: confirmed` is a match-set defect on our own schema, TWO changes with different costs: record a referent (hash+size+delta) per confirmation -- one command, works now, Brunel did it by hand on one entry; rename the field -- 235-card migration, decide at leisure; a half-fix raises trust without earning it. Also Celes owes Finn 2 items (S69).
7. **If Cal spawned:** read-back backlog must be RE-DERIVED from Cal's scratchpad state (Brunel withdrew the "5"/"2" counts as unmeasured); Volta's 3 S67 candidates + 3 read-backs unfiled; pronoun sweep queued (they/them default, 140 files / 771 occurrences measured, cannot be pattern-replaced, convention now in `wiki/CLAUDE.md`); courier `inboxes_dir`-never-logged defect unfiled (Brunel's surface); freshness-sweep defect (`last-verified` stamps fresh over non-discriminating re-checks) is Cal's own pass to fix.
