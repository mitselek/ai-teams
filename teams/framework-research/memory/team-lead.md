...
- **State of play:** S50 closed 2026-06-12 ~18:10 — **STATIONMASTER IS LIVE AND FIELD-VERIFIED.** Hub deployed+healthy on prod-llm (michelek@10.100.136.162, ~/stationmaster at 909bbe9, image stationmaster:1.0.0, port 2222, sm-state volume). ALL 8 TASKS CLOSED. #4 evidence: 15/15 live matrix both identities, bidirectional deposit/collect/ack, real courier inject+ledger cycle on live hub, channel-is-identity confirmed, 3-state error ladder, §8 Path.home() field-passed. Survived a 5-min Anthropic session-limit interrupt mid-deploy (~17:15; `interrupted` idle-storm = its signature; agents recovered with state intact).
- **Hub roster (5 keys):** alpha+beta (scratch), framework-research (PROD — Hopper's `~/.ssh/sm_framework-research` on dev box), fr-test (Herald's `~/.ssh/sm_fr_test`), **apex-research (CROSS-TEAM, registered, ephemeral-key rotate-on-restart policy accepted v1)**. Host-key fingerprint SHA256:CNcFjOxr8vREOueOS8nxJN8W3LaQHet62du+PHyK13U. Reciprocal FR⇄fr-test grants live.
- **APEX THREAD — ROUTE LIVE + EXERCISED (closed 2026-06-15 ~06:48Z, Herald respawn).** apex = customer #2 (Schliemann, their-PO-authorized; CLI 2.1.173). FR⇄apex grants SYMMETRIC both directions. First genuine two-team round-trip PROVEN: FR→apex consignment 67a3c27d3c0c5ec6 accepted+collected+acked (custody-transfer order honored); apex→FR 4627f4fa (reachability) + 6cdfa0c4 (validation) collected, injected as apex-research-ghost (anti-spoof), acked. Hub clean both ways (waiting_for_me:{}, deposited_uncollected:{}). Full courier (46KB) delivered to apex via manual fcntl-append (bypassed dead daemon). **S51 apex follow-ups:** apex builds/adapts their own courier side (they have the file); apex to report drain-on-delivery datapoint on 2.1.173 for TRUTHS.md; expect periodic apex key-rotation re-registration (ephemeral ~/.ssh). Ghost-bridge v2: FR-side daemon DEAD (stale PID 36772), transport alive, 34-entry apex backlog COPIED read-only to `~/.stationmaster-t4/apex-fr-backlog-copy.json` (UNTRIAGED — idx 26-32 = dupe-root-cause exchange, T1.b-relevant); SEQ-T1 "anomaly" resolved = read-flag-on-forward, no phantom.
- **Two deploy-blockers found+fixed in flight** (f022fed host-key gen; 909bbe9 nologin shell — both invisible to build, caught at up/smoke). Process learnings ALL FILED (wiki 120→132): artifact-freeze pattern, standby fix-then-flag 4-condition rule, hard-gate/sanction-binds-to-state, completed-flag=evidence, tmpfs gate-trap, registered-two-meanings contract (S51 design-review tagged). NEW meta-pattern observed, unfiled: operator's-latest-hub-state-confirm-beats-team-lead-instruction (4-message key-registration crossing resolved by ground-truth-wins).
- **Carry:** [DEFERRED — PO ratification] fan-out routing amendment (Herald's scratchpad, 3 candidates); PO decision pending: apex infra-level ~/.ssh mount vs rotate-on-restart; CLI substrate re-validation (TRUTHS 2.1.170 vs FR 2.1.175/apex 2.1.173) before production reliance on courier S2/S3 facts; ghost-bridge v2 decommission after apex cutover; backlog triage; A1 audit OVERDUE; GitHub retention-flip issue (PO go pending); D10 amendments fold-in; phantom-brunel; ELEX; hr-devs relay (now: invite as 3rd stationmaster customer?).

### NEXT-SESSION BOOT (re-orient instructions for S51)

1. Read `startup.md` first (always). Steps 1–5 (Sync → Reset team state → Restore inboxes → wait for PO direction before spawning).
...
4. **First substrate check:** is the hub still up? `ssh -i ~/.ssh/sm_framework-research -T -p 2222 sm@10.100.136.162` + ping conversation → expect team=framework-research. If down: Hopper-grade investigation on prod-llm (`docker ps`, logs) before anything else.
5. **Apex route is ALREADY LIVE (done S50 close).** If apex sends new traffic or their drain-on-delivery datapoint over the bridge: spawn Herald to handle/collect + relay the TRUTHS.md datapoint to Cal. If apex requests key-rotation re-registration (ephemeral ~/.ssh): spawn Hopper for the idempotent `sm-register apex-research <new-key>`. Their courier-side build is theirs to do (they have the file).
6. **If PO wants the v2→stationmaster cutover:** spawn Herald (+ Hopper for any hub-side ops) — migrate any remaining ghost-bridge pairs to grants, decommission the dead v2 daemon (stale PID 36772 on this box), triage the 34-entry backlog copy first (`~/.stationmaster-t4/apex-fr-backlog-copy.json`).
7. **If PO rules on the fan-out routing amendment:** fold the resolution into `wiki/gotchas/courier-originates-routing-protocol-leaves-undefined.md` (Cal) + courier v1.1 if needed (Herald). Candidates A/B/C in Herald's scratchpad, none pre-recommended.
8. **If PO gives the go on the GitHub retention-flip issue:** draft from TRUTHS.md I-1 + probe-1b evidence + 2.1.166–175 bracket; PO reviews before filing.
9. **If a CLI version change is detected at startup** (vs 2.1.175): re-run probe-1b before trusting inbox-substrate claims; courier-hints S2 facts are version-coupled.
10. **If Cal spawns:** one unfiled meta-pattern from S50 — "operator's latest hub-state confirm beats team-lead instruction" (ground-truth-wins conflict rule, 4-message key-crossing worked example in S50 wrap below).

### Standing watch items going into session 51

- **Apex route LIVE (no longer a watch — done).** Residual: apex's drain-on-delivery datapoint (CLI 2.1.173) for TRUTHS.md when it arrives; apex key-rotation re-reg requests (routine). Their courier-side build is on their schedule.
- **Hub health on prod-llm** — `restart: unless-stopped` should hold it; any restart wipes nothing (state on volume). Apex key rotation requests arrive after THEIR container restarts (ephemeral ~/.ssh — expected, not suspicious).
...
- [DECISION — S50, Aen 17:07] ARTIFACT FREEZE on stationmaster/ until #7 closes — 3 mid-deploy version skews in <1h (bfe3060, f022fed, 0667dd1); aggregate pattern = the hazard. Deploy-blockers: surface-before-fix via me; improvements: hold post-#7.
- [DEFERRED — post-#7] `git rm teams/framework-research/poc/ghost-bridge/stationmaster/t6a-race-harness.py` (Brunel's duplicate, Python-only; Hopper's poc/ghost-bridge/t6a-race-harness.py is canonical — both primitives, gate-of-record). Brunel-requested, Aen-approved, queued behind the freeze. Brunel's lesson logged: stage the dispatch, let the owner build the artifact.
