...
- **S72 second task (2026-09-03):** **amendment note for Minot DRAFTED** -- `teams/framework-research/docs/paunvere-amendment-note-2026-09-03.md` (delivery header for Brunel/Hopper + 38-line note body verbatim). Hopper's site visit (ops-log `2026-09-03T13:24`) confirmed my finding #2 the hard way: container runs PRE-amendment text; PO's `929ba8a` (08-31 13:22) postdates the image build (11:07) and TEAM_ROOT seed (11:14) by 2h08m, nothing failed in transit. Minot spent 09-03 enforcing the withdrawn gate -- declined Joosep 3x, declined the PO's own reply as probable social engineering (footer contradicted a claim), emailed Ruth.Tyrk directly, cleared 4 tests narrowly on her 09:56 yes. Note owns OUR slip, affirms the conduct was correct against the text held, affirms Ruth's clearance stands, no re-litigation. Placement `~/work/paunvere/AMENDMENTS.md` + 2 inserted lines in startup.md (stamp under the "read first" line + read-order item 2). **Stamp format `A<n>` self-checks against the scratchpad summary header** -- free check, and it drags the stale header rewrite along with it. English (common-prompt:80), with the Joosep one-liner supplied in Estonian. My other 8 fixes explicitly deferred to `A2`, named in the note. [GOTCHA] A rebuild does NOT deliver an amendment on this substrate: `SRC==STAMP` + `DST!=SRC` (git init inside TEAM_DST puts DST off STAMP from boot 2), so Step 9c's re-seed branch is unreachable by construction -- delivery is a hand-applied `diff -r`. [PATTERN -- Cal queue] **Amendment-level stamp anchored to an artifact the agent already reads and already rewrites** beats a changelog nobody is obliged to open.
- **S72 third task (2026-09-03 14:10) -- A1 SHIPPED into the deployed package** (PO sanctioned, scope grant for that task only): new `designs/deployed/joosep/teams/paunvere/AMENDMENTS.md` (38 lines, note body verbatim; item 2 softened to "confirm your summary header is current" -- Hopper found Minot rewrote it at shutdown 13:58) + `startup.md` patched 3 places (stamp line after the read-this-first line; `AMENDMENTS.md` as read-order item 2 with 3-6 renumbered; CLI pin "installs 2.1.250 (2.1.178+)" -> "runs 2.1.178 or later", my own W7 watch item). LF verified by explicit CR scan (repo is `text=auto`, Brunel hashes blobs on RC). md5 after: AMENDMENTS `4360afc2`, startup `e29fe123`->`97583e36`; the three amended files untouched at README `36455e70` / common-prompt `5adad74f` / saxby `01114426`. Also synced the same one-line softening into my docs draft and flagged that file as outside the grant for Aen's call. Aen commits; I did no git.
