...
- **Current state:** S44 (2026-06-06) — **DESIGN LEAD. Task #3 spec DONE+committed (`5ca637f`)+#42 comment LIVE+FROZEN. NOW: Task #4 PERSONA CASTING — Phase 2 DRAFT COMPLETE, awaiting team-lead review before commit.** Wrote full tree `designs/new/entu-consultant-agents/`: 4 domains + formula A/B pair. data-lifecycle=Pérotin (FULL competencies.yaml, 18 claims from Finn harvest), auth=Phileas-Fogg (deferred), schema=Linnaeus (deferred), formula=Hamblin+Harrison A/B (both deferred). Each dir: persona.md+prompt.md+synergy.md+competencies.yaml. Plus `formula-engine-EXPERIMENT.md`. VERIFIED: formula prompt.md/synergy.md BYTE-IDENTICAL across variants (control holds); guardrail verbatim in all 5 personas; attribution all 21 files; lint-clean (MD060-only). [DECISION] formula A/B = PO research call: Hamblin (domain-fact-famous=RPN-inventor, NOT sanitized=treatment) vs Harrison (method-famous=control); guardrail held CONSTANT/standard (NOT doubled for Hamblin) — persona = sole variable; claims deferred both. AWAITING: Cal sign-off on competencies.yaml provenance-header shape (global index store not materialized → honest `@ <pending>` placeholder) + conformance. [FREEZE 16:49] Task #3 docs frozen at HEAD — NO micro-refinements, only flag substantive problems. Protocol-A pattern FILED `wiki/patterns/citation-backed-beats-posture-backed-when-fact-is-subtle.md` (stage-2 PARTIAL, confirms on Finn read-back).
