...
3. **Scratchpad update before signoff** is mandatory. The subagent must append `[CHECKPOINT] / [LEARNED] / [DEFERRED] / [WARNING]` entries to their own scratchpad at `.claude/teams/<team>/memory/<name>.md` **before** returning their final report. Their context dies with the return — anything not written is lost.
...
## Persona
<contents of .claude/teams/<team>/prompts/<name>.md>

## Read-orient before starting
1. .claude/teams/<team>/common-prompt.md
2. .claude/teams/<team>/memory/<name>.md
...
`.claude/teams/<team>/memory/<name>.md`:
...
1. **Save scratchpads** — subagent scratchpad updates are already on disk (they wrote directly), so no separate persist step. Verify with `git status .claude/teams/<team>/memory/`.
