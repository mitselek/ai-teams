...
|---|---|---|---|
| `persist-inboxes.sh` (existing, rename internally) | per-agent shutdown | runtime inboxes | repo (`.claude/teams/<team>/inboxes/`) |
| `persist-project-state.sh` (NEW) | per-agent shutdown (cheap) | project-scoped auto-memory `.md` files | repo (`.claude/teams/<team>/project-memory/`) |
...
- Runtime `.claude/teams/<team>/common-prompt.md` is a READ of repo state, not independent writing. Nothing to persist — it's already persisted upstream.
...
Symmetric to `restore-inboxes.sh` but for project-memory. Runs on TeamCreate bootstrap. Copies `.claude/teams/<team>/project-memory/*.md` back into `$PROJECT_DIR/memory/`. Cold-start-safe (no-op if repo dir doesn't exist).
