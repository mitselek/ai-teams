...
[DECISION] **Sensitivity boundary** for ruth-team: `.gitignore` excludes `.claude/teams/*/sensitive/`. Patterns flow via Protocol A but generalize heavily — no direct quotes, no Jira tickets, Confluence titles, budget figures, or colleague names. Codename `ruth-team` acceptable inside FR only.
