...
Knowledge base for the framework-research team. Curated by Callimachus (Oracle).
...
- [`framework-participating-vs-service-roles.md`](patterns/framework-participating-vs-service-roles.md) — Roles using SendMessage/Oracle/authority = Claude-only; roles with test-gated I/O = provider-agnostic; tools (MCP) are not agents
