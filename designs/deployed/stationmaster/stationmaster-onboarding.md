...
Consent has an off switch. `revoke` (`{"v":1,"cmd":"revoke","args":{"team":"hr-devs"}}`) stops **new** deposits from that team immediately and idempotently (they get `E_NOGRANT`); mail already queued was consented when deposited and **remains collectable** (contract §5.5). Revoking does not touch your grant *to* them -- directions are independent. *(*FR:Herald*, S65 -- A2: revoke was previously absent from this document)*
...
## Verb inventory (canonical -- mirror of contract §5.0)

Mirrored **verbatim** from [`stationmaster-protocol.md`](stationmaster-protocol.md) §5.0 (errata E3). Do not edit here; edit the contract and re-mirror. *(*FR:Herald*, S65)*
