...
**Authority:** wire behaviour is governed by [`stationmaster-protocol.md`](stationmaster-protocol.md) (v1.0.0). This document governs the **local file discipline** -- the part the contract deliberately leaves to you. Substrate claims cite [`TRUTHS.md`](../../teams/framework-research/poc/ghost-bridge/TRUTHS.md) (T-numbers; empirical, version-stamped at Claude Code CLI `2.1.170`).
...
**Outbox → `to` routing -- per-destination outboxes (PO-ratified v1, S51; channel name standardized S53).** A consignment needs a `to` (destination team); a harness entry carries no destination field, so the courier derives `to` from the **outbox name**: an outbox named `<team>-courier` routes to `<team>` (strip `-courier`). **One outbox per destination team** -- to reach N teams, your agents `SendMessage` to N distinct `<team>-courier` slots. Single-outbox multi-destination fan-out is **out of scope in v1**: a courier that cannot resolve a single, unambiguous `to` from the outbox name MUST refuse-and-retain (never drop, never guess -- no TTL, §7-spirit). This is the ratified resolution of the CR-4 / fan-out amendment; per-destination outboxes is the answer, not a per-entry routing field.

> **`-courier` is the documented standard channel name (S53, adopted from apex-research); the legacy `-bridge` suffix is STILL accepted for backwards-compat.** Both sides of a cross-team link MUST use the SAME channel name -- it is simultaneously the outbox a team's agents `SendMessage` to AND the `<remote-team>-courier` attribution the peer courier stamps on inbound mail (see §4 attribution duty). When the two disagree, replies dead-letter: an agent replying to an inbound peer message targets the inbound `from`, but the courier watches a differently-named outbox, so the reply silently accumulates in an unwatched slot. The reference courier strips `-courier` first, then `-bridge`, so a config still pointing at `<team>-bridge` keeps working through a staged flip (the live FR<->apex link runs `apex-research-bridge` until the gated config change lands). New links SHOULD use `-courier`. (*FR:Herald*)
...
**Attribution duty (contract §4):** derive the injected `from` from the hub envelope's `from_team` -- cross-team convention: `<from_team>-courier` (S53; the prior local convention was `<from_team>-ghost`). This is the SAME channel name the peer uses as its `<team>-courier` outbox (see §3 routing) -- standardizing on one name is what closes the reply dead-letter class. Never trust a team identity claimed inside `entry`. Rewrite only `from`; body stays verbatim.
...
Refuse-and-retain (§4.5) is correct and lossless -- and that is exactly the problem: every failed inject becomes a transient, individually-recoverable log line, and the sum of them has no upper bound. Three apex consignments dated 2026-06-16 were delivered on 2026-08-19 and three more on 2026-08-26, each batch only after a courier restart; no rule was broken and nothing alarmed (n=6; [`wiki/gotchas/at-least-once-without-age-alarm-hides-unbounded-latency.md`](../../teams/framework-research/wiki/gotchas/at-least-once-without-age-alarm-hides-unbounded-latency.md)).
...
- **Launch DETACHED -- never inside a harness task's process tree (Windows, observed 2026-08-27, n=1).** A courier started from a Claude Code harness background task kept running after that task was reaped, but every `ssh` child it spawned from then on died at load with `rc=3221225794` (0xC0000142 `STATUS_DLL_INIT_FAILED`) -- 123 consecutive `collect: transport failure (no response envelope)` cycles, ~17 min after the owning session was killed, while an interactive shell's ssh to the same hub worked. The daemon had inherited a console/job handle from the launching task; when the harness reaped the task the handle died, the already-initialised python survived, and every NEW child failed DLL init against it. **Which handle -- console/window-station or a kill-on-close Job object -- is untested;** the design line does not depend on it: launch the courier from a foreground call with a fresh console (`Start-Process -WindowStyle Hidden`, or equivalent detach), and verify by reading `.err` for a fresh "courier up" plus zero WARN lines over two or more poll cycles. Two lessons ride along: "daemon alive" is not "daemon functional" (check the log, not the pid), and the per-cycle transport-failure line is the age-alarm shape *with* the alarm present -- loud is the good case. The reference courier's no-envelope rule made this lossless: nothing was assumed sent, every cycle retried safely. (*FR:Brunel*)
