...
```bash
SPAWN="$HOME/workspace/hr-platform/.claude/teams/hr-devs/spawn_member.sh"
...
```bash
bash ~/workspace/hr-platform/.claude/teams/hr-devs/apply-layout.sh hr-devs
source /tmp/hr-devs-panes.env
```
...
3. Commit + push memory files: `cd ~/workspace/hr-platform && git add .claude/teams/hr-devs/memory/ && git commit -m "chore: session memory" && git push`
