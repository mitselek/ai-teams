...
# Respawn: ar-remove-member <agent> + re-Agent(). See issue #60.
# (*FR:Brunel*)
# ──────────────────────────────────────────────────────────────────────────────
...
TMUX_SESSION="${2:-screenwerk-dev}"

TEAM_NAME="screenwerk-dev"
...
SPAWN_SCRIPT=$(mktemp /tmp/spawn-sw-XXXXXX.sh)
{
    echo '#!/usr/bin/env bash'
    echo 'export CLAUDECODE=1'
    echo 'export CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1'
    echo 'export CLAUDE_CODE_NO_FLICKER=1'
    echo "export CLAUDE_ENV_ID=\"SW-DEV\""
...
if [[ -n "$TARGET_PANE" ]]; then
    ...
    tmux split-window -t "$TMUX_SESSION" -h -l '70%' -c "$WORK_DIR" "bash $SPAWN_SCRIPT"
