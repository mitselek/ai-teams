...
- **All issue text is English** (Mihkel's decision), identical label set across all four product repos: core `epic`/`task`/`ready`/`blocked`/`needs-po`; optional `priority:P0..P2`, `epic:<slug>`, `product:<name>`; per-product `area:*` at your discretion. Cross-product block = plain `blocked` + a `Blocked on <owner>/<repo>#<N>` line, escalated to Henry.
